...
def reset_interface():
    ...
    canvas[canvas_y:canvas_y+canvas_h, canvas_x:canvas_x+canvas_image_w] = cv2.resize(image_list[main_index], (canvas_image_w, canvas_h), interpolation=cv2.INTER_CUBIC)
...
while True:
    ...
    if crop_index < crop_num:
        ...
        for i in range(len(image_list)):
            ...
            tcanvas[crop_y:crop_y+crop_size, crop_x:crop_x+crop_size] = cv2.resize(crop, (crop_size, crop_size), interpolation=cv2.INTER_CUBIC)
    cv2.imshow('SR Viewer', tcanvas)
    key = cv2.waitKey(1)
    if key == ord('a'):
